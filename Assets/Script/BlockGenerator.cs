...
public class BlockGenerator : MonoBehaviour
{
    ...
    public GameObject BP = new GameObject(); // 블럭들의 부모를 담아놓음
    public GameObject Arrangement = new GameObject(); // 현재 블럭 인스턴스를 받음  
    public bool CheckException; // 겹침 예외판정
    BlockDrag BD = new BlockDrag(); // 블럭드래그 스트립트 컴포넌트 캐싱
    Transform PlayerPos; // 플레이어 위치 값 정보
    SelectBlock_UI SelectBlock = new SelectBlock_UI();
    List<float> Sort_PS; // oder in layer 순위 설정을 위해 y축 좌표를 리스트에 넣어둠
    List<Vector3> Search_PS; // 맵에 있는 모든 타일과 블럭 좌표 리스트

    public SpriteRenderer BlockColorAlpha = new SpriteRenderer(); // Arrang_obj 함수에서 Block 색의 Alpha 값을 받아옴
    ...
    {           
       ...
       // ClickNum = 0;
        ...
        BlockPrefab[0] = Resources.Load("BlockPrefab1") as GameObject;
        BlockPrefab[1] = Resources.Load("BlockPrefab2") as GameObject;
        BlockPrefab[2] = Resources.Load("BlockPrefab3") as GameObject;
        BlockPrefab[3] = Resources.Load("BlockPrefab4") as GameObject;
        BlockPrefab[4] = Resources.Load("BlockPrefab5") as GameObject;
        BlockPrefab[5] = Resources.Load("BlockPrefab6") as GameObject;
        BlockPrefab[6] = Resources.Load("BlockPrefab7") as GameObject;

        BlockPrefab_R = new GameObject[7, 3]; //회전시 생성되는 블록
        BlockPrefab_R[0, 0] = Resources.Load("BlockPrefab1") as GameObject;
        BlockPrefab_R[1, 0] = Resources.Load("BlockPrefab2_R1") as GameObject;
        BlockPrefab_R[2, 0] = Resources.Load("BlockPrefab3_R1") as GameObject;
        BlockPrefab_R[2, 1] = Resources.Load("BlockPrefab3_R2") as GameObject;
        BlockPrefab_R[2, 2] = Resources.Load("BlockPrefab3_R3") as GameObject;
        BlockPrefab_R[3, 0] = Resources.Load("BlockPrefab4_R1") as GameObject;
        BlockPrefab_R[3, 1] = Resources.Load("BlockPrefab4_R2") as GameObject;
        BlockPrefab_R[3, 2] = Resources.Load("BlockPrefab4_R3") as GameObject;
        BlockPrefab_R[4, 0] = Resources.Load("BlockPrefab5_R1") as GameObject;
        BlockPrefab_R[5, 0] = Resources.Load("BlockPrefab6_R1") as GameObject;
        BlockPrefab_R[6, 0] = Resources.Load("BlockPrefab7_R1") as GameObject;
        BlockPrefab_R[6, 1] = Resources.Load("BlockPrefab7_R2") as GameObject;
        BlockPrefab_R[6, 2] = Resources.Load("BlockPrefab7_R3") as GameObject;
    }
